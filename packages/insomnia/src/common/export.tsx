...
export const exportProjectToFile = (
  ...
  if (!workspacesForActiveProject.length) {
    showAlert({
      ...
      message: (
        <>
          There are no workspaces to export in the{" "}
  ...
  showSelectExportTypeModal({
    onDone: async (selectedFormat) => {
      const exportPrivateEnvironments =
        await showExportPrivateEnvironmentsModal();
      ...
      try {
        switch (selectedFormat) {
          case VALUE_HAR:
            stringifiedExport = await exportWorkspacesHAR(
          ...
          case VALUE_YAML:
            stringifiedExport = await exportWorkspacesData(
              workspacesForActiveProject,
              exportPrivateEnvironments,
              "yaml"
            );
          ...
          default:
            throw new Error(
              `selected export format "${selectedFormat}" is invalid`
            );
        }
      } catch (err) {
        showError({
  ...
  showSelectExportTypeModal({
    onDone: async (selectedFormat) => {
      ...
      for (const requestId of requestIds) {
        ...
        if (
          workspace == null ||
          workspaceLookup.hasOwnProperty(workspace._id)
        ) {
          continue;
        }
      ...
      try {
        switch (selectedFormat) {
          case VALUE_HAR:
            stringifiedExport = await exportRequestsHAR(
              requests,
              exportPrivateEnvironments
            );
            break;
          ...
          case VALUE_YAML:
            stringifiedExport = await exportRequestsData(
              requests,
              exportPrivateEnvironments,
              "yaml"
            );
