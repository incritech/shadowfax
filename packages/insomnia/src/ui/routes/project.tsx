...
export const indexLoader: LoaderFunction = async ({ params }) => {
  ...
  if (prevOrganizationLocation) {
    ...
    if (match && match.params.organizationId && match.params.projectId) {
      const existingProject = await models.project.getById(
      ...
      if (existingProject) {
        ...
        return redirect(
          `/organization/${match?.params.organizationId}/project/${existingProject._id}`
        );
      }
    }
  }
...
export const projectLoader: LoaderFunction = async ({ request }) => {
  try {
    ...
    const activeProject = await models.project.getById(
      projectPath.params.projectId
    );
    invariant(
      activeProject,
      `Project was not found ${projectPath.params.projectId}`
    );
...
const ProjectRoute: FC = () => {
  ...
  const workspacesWithPresence = workspaces.map((workspace) => {
    const workspacePresence = presence
      .filter(
        (p) => p.project === activeProject.remoteId && p.file === workspace._id
      )
            ...
            <div className="flex flex-1 flex-col overflow-hidden divide-solid divide-y divide-[--hl-md]">
                                ...
                                >
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    >
                                      ...
                                      <p className="pt-2">
                                        End-to-end encrypted (E2EE) and synced
                                        securely to the cloud, ideal for
                                        collaboration.
                                      </p>
                                    </Radio>
                                    <Radio
                                    ...
                                    >
                                      ...
                                      <p className="pt-2">
                                        Stored locally only with no cloud. Ideal
                                        when collaboration is not needed.
                                      </p>
                                    </Radio>
                                  </div>
                                </RadioGroup>
              ...
              <div className="flex flex-shrink-0 flex-col py-[--padding-sm]">
                ...
                >
                  ...
                  <span className="truncate">Help and feedback</span>
                </Button>
              </div>
            </div>
          }
