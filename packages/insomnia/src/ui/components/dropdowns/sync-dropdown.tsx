...
export const SyncDropdown: FC<Props> = ({ vcs, workspace, project }) => {
  ...
  const { organizationId, projectId } =
    useParams<{ organizationId: string; projectId: string }>();
  const navigate = useNavigate();
  const { activeWorkspaceMeta, syncItems } = useRouteLoaderData(
    ":workspaceId"
  ...
  const refreshVCSAndRefetchRemote = useCallback(async () => {
    if (!vcs.hasBackendProject() && isRemoteProject(project)) {
      ...
      const matchedBackendProjects = remoteBackendProjects.filter(
        (p) => p.rootDocumentId === workspace._id
      );
      setState((state) => ({
        ...state,
  ...
  async function handleSetProject(backendProject: BackendProjectWithTeam) {
    ...
    const pulledIntoProject = await pullBackendProject({
      vcs,
      backendProject,
    ...
    if (pulledIntoProject.project._id !== project._id) {
      ...
      console.log(
        "[sync] collection has been moved to the remote project to which it belongs",
        {
          collection: {
            id: workspace._id,
            name: workspace.name,
  ...
  async function handleSwitchBranch(branch: string) {
    try {
      ...
      if (branch === DEFAULT_BRANCH_NAME) {
        const defaultBranchHistoryCount = await vcs.getHistoryCount(
          DEFAULT_BRANCH_NAME
        );
        ...
        if (!defaultBranchHistoryCount && historyCount) {
          delta.remove = delta.remove.filter(
            (e) => e?.type !== models.workspace.type
          );
        }
      }
      await db.batchModifyDocs(delta as unknown as Operation);
  ...
  const canCreateSnapshot =
    Object.keys(status.stage).length > 0 ||
    Object.keys(status.unstaged).length > 0;
  ...
  if (!vcs.hasBackendProject()) {
    return (
      <div>
        ...
        >
          <DropdownSection>
            <DropdownItem key="gitSync" aria-label="Setup Git Sync">
              <Button
                variant="contained"
                bg="surprise"
                onClick={async () => {
          ...
          <DropdownSection
            ...
            items={
              remoteBackendProjects.length === 0 ? emptyDropdownItemsArray : []
            }
            title={syncMenuHeader}
          >
            {(p) => (
              <DropdownItem key={p.id} textValue={p.name}>
          ...
          <DropdownSection
            aria-label="Sync Projects List"
            items={
              remoteBackendProjects.length !== 0 ? remoteBackendProjects : []
            }
            title={syncMenuHeader}
          >
            {(p) => (
              <DropdownItem key={p.id} aria-label={`Pull ${p.name}`}>
                <ItemContent
                  icon="cloud-download"
                  label={
                    <>
  ...
  const pullToolTipMsg = canPull
    ? `There ${behind === 1 ? "is" : "are"} ${behind} snapshot${
        behind === 1 ? "" : "s"
      } to pull`
    : "No changes to pull";
  const pushToolTipMsg = canPush
    ? `There ${ahead === 1 ? "is" : "are"} ${ahead} snapshot${
        ahead === 1 ? "" : "s"
      } to push`
    : "No changes to push";
  const snapshotToolTipMsg = canCreateSnapshot
    ? "Local changes made"
  ...
  return (
    <div>
                    ...
                    <Fragment>
                      ...
                      >
                        <Icon
                          ...
                          className={`transition-colors ${
                            canPull ? "" : "opacity-50"
                          } ${loadingPull ? "animate-pulse" : ""}`}
                        />
                      </Tooltip>

                      <Tooltip
                        message={pushToolTipMsg}
                        delay={800}
                        position="bottom"
                      >
                        <Icon
      ...
      >
        ...
        <DropdownSection aria-label="Sync Branches List" title={syncMenuHeader}>
        ...
        >
          {visibleBranches.map((branch) => {
            ...
            return (
              <DropdownItem key={branch} aria-label={branch}>
                <ItemContent
